import { Flex, Image, Text, Button, Link, SlideFade, useColorModeValue } from '@chakra-ui/react'
...
const Track = ({ track }: { track: SpotifyTrack }) => {
  ...
  const isSelected: boolean = selectedTracks.some(({ uri }) => uri === track.uri )
  ...
  return (
    <SlideFade in unmountOnExit>
      <Flex align="center" my={2} p={2} borderRadius="lg" _hover={{ bg }}>
        <Image w={'64px'} h={'64px'} borderRadius={2} src={image?.url} alt={track.name} title={track.name} />
